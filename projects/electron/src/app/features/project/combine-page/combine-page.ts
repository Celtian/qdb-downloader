...
@Component({
  ...
  templateUrl: './combine-page.html',
  styleUrl: './combine-page.css',
})
export class CombinePage {
  ...
  protected async prepare(): Promise<void> {
    ...
    if (this.selectedTeamCount() < 2) {
      this.error.set('Choose teams from at least two providers.');
      return;
    }
    this.busy.set(true);
    this.error.set('');
  ...
  protected async commit(): Promise<void> {
    ...
    this.snackBar.open(
      `${result.value.team.name} combined. ${result.value.addedPlayers} players added, ${result.value.updatedPlayers} updated, ${result.value.deletedPlayers} removed.`,
      'Dismiss',
      { duration: 6000 },
    );
    await this.router.navigate(['../combined/teams'], { relativeTo: this.route });
  }
